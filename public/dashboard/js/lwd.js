class LoginWithDiscord {
  ...
  async login(clientID, ...scopes) {
    return new Promise((resolve, reject) => {
      ...
      let popout = window.open(url, 'Login', `width=400,height=600`);
      popout.focus();
  ...
  async fetchGuild(id) {
    return new Promise((resolve, reject) => {
      ...
      Util.requestJSON('GET', `https://discordapp.com/api/v6/guilds/${id}`, {
        Authorization: `Bot NjE3NDAwOTQ5NzMxNzUzOTg3.XZlnag.1tpVBqGTMUdZbFhOBMBW2FxqF5Q`
      }).then((guild) => {
        resolve({
          name: guild.name,
          icon: `https://cdn.discordapp.com/icons/${id}/${guild.icon}.png`
        });
      }).catch(reject);
    });
  }
