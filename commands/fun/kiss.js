...
module.exports.run = async (bot, message, args) => {
    ...
    if (!args[0]) {
        const ghembed = new Discord.RichEmbed()
            .setColor(`RANDOM`)
            .setTitle(`${message.author.username} se besó asi mismo...! (bicho raro)`)
    ...
    if (!message.mentions.members.first().user.username === message.isMentioned(message.author)) {
        const hembed = new Discord.RichEmbed()
            .setColor(`RANDOM`)
            .setTitle(`${message.author.username} le dio un beso a ${message.mentions.members.first().user.username} ¡Que dulce!`)
...
exports.config = {
  name: "kiss",
  aliases: [],
  description: "Comando para dar un beso"
};