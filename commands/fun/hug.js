...
module.exports.run = async (bot, message, args) => {
    ...
    if (!args[0]) {
        const ghembed = new Discord.RichEmbed()
            .setColor(0xFF0000)
            .setTitle(`${message.author.username} se abrazó a sí mismo ...! (bicho raro)`)
    ...
    if (!message.mentions.members.first().user.username === message.isMentioned(message.author)) {
        const hembed = new Discord.RichEmbed()
            .setColor(0xFF0000)
            .setTitle(`${message.author.username} le dio un abrazo a ${message.mentions.members.first().user.username}  ¡Que dulce!`)
...
exports.config = {
  name: "hug",
  aliases: [],
  description: "Comando para dar un abrazo"
};