...
module.exports.run = async (client, message, args, ops) => { //Collecting info about command
  ...
  if (config[message.guild.id].djonly == true) {
    if (!message.member.roles.some(r=>config[message.guild.id].djroles.includes(r.id))) return message.channel.send({ 
      embed: {
        "title": "Error", 
        "description": "En este servidor, el modo DJOnly está activado\nNo tienes ningún role de DJ, así que no puedes tocar canciones.\n*Para ver la lista de roles de DJ, use `" + config[message.guild.id].prefix + "dj`*", 
      } 
    });
  }
  
  var song = args[0];
  config = JSON.parse(fs.readFileSync("./settings/guilds.json", "utf8")); //Config file
  var streamOptions = {
    seek: 0,
  ...
  if (!message.member.voiceChannel) {
    return message.channel.send({
      embed: {
        "title": "¡Únase al canal de voz primero!",
        "color": 0xff2222
      }
    }).then(msg => {
  ...
  if (!song) {
    return message.channel.send({
      embed: {
        "title": "¡Ingrese la URL o el nombre de la canción!",
        "color": 0xff2222
      }
    }).then(msg => {
      if (config[message.guild.id].delete == 'true') {
  ...
  if (!data.dispatcher) {
    ...
    message.channel.send(new Discord.RichEmbed()
      ...
      .setDescription("Agregado a la cola **" + info.title + "**")).then(msg => {
      if (config[message.guild.id].delete == 'true') {
        msg.delete(config[message.guild.id].deleteTime);
      }
    });
...
async function play(client, ops, data, streamOptions) {
  ...
  client.channels.get(data.queue[0].announceChannel).send(new Discord.RichEmbed()
    ...
    .setDescription("Ahora escuchando: **" + data.queue[0].songTitle + "**")).then(msg => {
